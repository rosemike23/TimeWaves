# Wavelet line output modification by micha
...
def cwtline(data, scales, wavelet, sampling_period=1., method='conv', axis=-1, top_index=[]):
    ...
    # def cwt(data, scales, wavelet, sampling_period=1., method='conv', axis=-1, line=-1):
    
    # accept array_like input; make a copy to ensure a contiguous array
    # 输入处理：检查输入数据的数据类型，转换为np数组，同时根据数据类型选择适当的复数数据类型
    dt = _check_dtype(data)
    data = np.asarray(data, dtype=dt)
    dt_cplx = np.result_type(dt, np.complex64)
    
    top_index = top_index # 输入欲求的频率值
    precision = 10
    cwt_freq = scale2frequency(wavelet, scales, precision) / sampling_period
    scales_reform = np.array([])
    for i in range(top_index.shape[0]):
        temp = np.where(abs(cwt_freq - top_index[i]) < 1e-3)
        scales_reform = np.append(scales_reform, scales[temp])
    # print(f"scales before :{scales.shape} ")
    scales = scales_reform
    # print(f"scales after :{scales.shape} ")
